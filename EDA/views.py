...
def FileUpload(request):
    if request.method == 'POST':
        try:
            ...
            messages.error(request, "Error occured during file upload")
...
def DownloadDataset(request, id):
    try:
        ...
        messages.error(request, "unable to download dataset")
        return HttpResponse("File not found.", status=404)
...
def generate_analysis_report(request, id):
    try:
        ...
        messages.error(request, "unable to download report")
        return None